import {
  BaseInput
} from "./chunk-OJQ4L36R.js";
import {
  InputText
} from "./chunk-BGU7KAOR.js";
import {
  NG_VALUE_ACCESSOR
} from "./chunk-CSV65KEI.js";
import {
  Button,
  Ripple
} from "./chunk-T5AXUQQB.js";
import {
  AutoFocus,
  CalendarIcon,
  ChevronDownIcon,
  ...
  blockBodyScroll,
  unblockBodyScroll
} from "./chunk-6EBAJMRW.js";
import "./chunk-AKMRTSBA.js";
import {
  BaseComponent,
  Bind,
  BindModule,
  PARENT_INSTANCE
} from "./chunk-GKPSMWXA.js";
import {
  BaseStyle,
  P as P2,
  W,
...
var p = (t, n) => {
  ...
  if (T2) for (var e of T2(n)) V2.call(n, e) && D(t, e, n[e]);
  return t;
};
...
function A(t, n) {
  ...
  }, [m, i] = o(v), [M, f] = o(E2), r = Math.max(...i.map((l, c) => l + m[c])), u = Math.max(...f.map((l, c) => l + M[c])), a, s3 = 0, d = 0;
  return n === v ? r > 0 && (a = v, s3 = r, d = i.length) : n === E2 ? u > 0 && (a = E2, s3 = u, d = f.length) : (s3 = Math.max(r, u), a = s3 > 0 ? r > u ? v : E2 : void 0, d = a ? a === v ? i.length : f.length : 0), { type: a, timeout: s3, count: d };
}
function $(t, n) {
...
function S(t, n = true, e = false) {
  if (!n && !e) return;
  let o = w(t);
  n && te2(t, "--pui-motion-height", o.height + "px"), e && te2(t, "--pui-motion-width", o.width + "px");
}
...
function tt(t, n) {
  ...
  let e = {}, o = false, m = {}, i = null, M = {}, f = (a) => {
    ...
    i == null || i();
    let { onBefore: s3, onStart: d, onAfter: l, onCancelled: c } = M[a] || {}, h = { element: t };
    if (o) {
      s3 == null || s3(h), d == null || d(h), l == null || l(h);
      return;
    }
    let { from: g, active: y, to: P3 } = m[a] || {};
    return S(t, e.autoHeight, e.autoWidth), s3 == null || s3(h), W(t, g), W(t, y), t.offsetHeight, P2(t, g), W(t, P3), d == null || d(h), new Promise((b) => {
      let C2 = $(e.duration, a), x2 = () => {
        P2(t, [P3, y]), i = null;
      }, R = () => {
        x2(), l == null || l(h), b();
      };
...
var Motion = class _Motion extends BaseComponent {
  ...
  handleBeforeEnter = (event2) => !this.destroyed && this.onBeforeEnter.emit(event2);
  handleEnter = (event2) => !this.destroyed && this.onEnter.emit(event2);
  handleAfterEnter = (event2) => !this.destroyed && this.onAfterEnter.emit(event2);
  handleEnterCancelled = (event2) => !this.destroyed && this.onEnterCancelled.emit(event2);
  handleBeforeLeave = (event2) => !this.destroyed && this.onBeforeLeave.emit(event2);
  handleLeave = (event2) => !this.destroyed && this.onLeave.emit(event2);
  handleAfterLeave = (event2) => !this.destroyed && this.onAfterLeave.emit(event2);
  handleLeaveCancelled = (event2) => !this.destroyed && this.onLeaveCancelled.emit(event2);
  constructor() {
    super();
    effect(() => {
...
var DatePicker = class _DatePicker extends BaseInput {
    ...
    switch (event2.which) {
      ...
      case 37: {
        ...
        if (prevCell) {
          ...
          if (k(focusCell, "p-disabled") || k(focusCell.parentElement, "p-datepicker-weeknumber")) {
  ...
  destroyMask() {
    ...
    for (let i = 0; i < bodyChildren.length; i++) {
      ...
      if (k(bodyChild, "p-datepicker-mask-scrollblocker")) {
  ...
  formatTime(date) {
    ...
    if (this.hourFormat == "12") {
      output2 += hours === 0 ? 12 : hours < 10 ? "0" + hours : hours;
    } else {
      output2 += hours < 10 ? "0" + hours : hours;
    }
    output2 += ":";
    ...
    if (this.hourFormat == "12") {
      output2 += date.getHours() > 11 ? " PM" : " AM";
    }
    return output2;
  }
  parseTime(value) {
  ...
  isNavIconClicked(event2) {
    return k(event2.target, "p-datepicker-prev-button") || k(event2.target, "p-datepicker-prev-icon") || k(event2.target, "p-datepicker-next-button") || k(event2.target, "p-datepicker-next-icon");
  }
  onWindowResize() {
  ...
  static ɵcmp = ɵɵdefineComponent({
    ...
    contentQueries: function DatePicker_ContentQueries(rf, ctx, dirIndex) {
      if (rf & 1) {
        ɵɵcontentQuery(dirIndex, _c02, 4)(dirIndex, _c1, 4)(dirIndex, _c2, 4)(dirIndex, _c3, 4)(dirIndex, _c4, 4)(dirIndex, _c5, 4)(dirIndex, _c6, 4)(dirIndex, _c7, 4)(dirIndex, _c8, 4)(dirIndex, _c9, 4)(dirIndex, _c10, 4)(dirIndex, _c11, 4)(dirIndex, _c12, 4)(dirIndex, PrimeTemplate, 4);
      }
      if (rf & 2) {
        let _t;
